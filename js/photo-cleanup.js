(function() {
	...
	function runPhotoCaption() {
        ...
        if (locate.indexOf('upload.php') > -1) {
            ...
            if (afpCredit !== false) {
                ...
                //new photo credit string as of 1/18/19
                var newCredit = "MediaNews Group";
                //check for new credit use
                var newCreditCheck = photoCred.search(newCredit);
                ...
                //checking for new photo credit
                } else if (newCreditCheck != -1) {
                    if (photoCred.match(/Photo By/)) {
                        photoCredNew = photoCred.replace('Photo By ', '').replace('/', ', ').replace('MediaNews Group/The Denver Post', 'The Denver Post');
                    } else if (photoCred.match(/Photo by/)) {
                        photoCredNew = photoCred.replace('Photo by ', '').replace('/', ', ').replace('MediaNews Group/The Denver Post', 'The Denver Post');
                    }
                    //removing mediaNews tag from caption text
                    caption.textContent = caption.textContent.replace('MediaNews Group/The Denver Post', 'The Denver Post');
                    document.querySelectorAll('[data-setting="description"] textarea')[0].value = caption.textContent;
                }else {
                    if (photoCred.match(/Photo By/)) {
                        //old check wont be used after 1/18/19
    ...
    //check if photo has credit already, and ask if you would like to proceed
    if (typeof document.querySelectorAll('.compat-field-credit td.field input[type="text"]')[0] != 'undefined') {
        //get photo credit input box
        var credit = document.querySelectorAll('.compat-field-credit td.field input[type="text"]')[0];

        //run check for credit field
        if (credit.value == ""){
            runPhotoCaption();
        }else{
            if (confirm("Photo has a credit already, Are you sure you want to proceed?...things could get messy.")) {
                runPhotoCaption()
            //ending if statement to check for empty photo credit
            }else{
                //do nothing they didn't want to proceed
            }
        }
    } //end check for previous photo credit
}());